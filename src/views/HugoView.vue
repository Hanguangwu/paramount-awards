...
onMounted(async () => {
  try {
    const res = await fetch('/data/hugo.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Hugo data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.hugo
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="hugo" />
