...
onMounted(async () => {
  try {
    const res = await fetch('/data/nobel.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Nobel data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.nobel
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="nobel" />
