...
onMounted(async () => {
  try {
    const res = await fetch('/data/turing.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Turing data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.turing
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="turing" />
