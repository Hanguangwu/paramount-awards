...
onMounted(async () => {
  try {
    const res = await fetch('/data/darwin.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Darwin data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.darwin
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="darwin" />
