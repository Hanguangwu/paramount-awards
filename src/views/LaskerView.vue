...
onMounted(async () => {
  try {
    const res = await fetch('/data/lasker.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Lasker data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.lasker
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="lasker" />
