...
onMounted(async () => {
  try {
    const res = await fetch('/data/oscar.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Oscar data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.oscar
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="oscar" />
