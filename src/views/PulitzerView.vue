...
onMounted(async () => {
  try {
    const res = await fetch('/data/pulitzer.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Pulitzer data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.pulitzer
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="pulitzer" />
