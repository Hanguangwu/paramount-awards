...
onMounted(async () => {
  try {
    const res = await fetch('/data/pritzker.json')
    winners.value = await res.json()
  } catch (e) {
    console.error('Failed to load Pritzker data', e)
  } finally {
    loading.value = false
  }
})

const meta = AWARD_META.pritzker
</script>
...
<template>
  <div class="award-page">
    ...
    <WinnerList v-else-if="winners.length" :winners="winners" award="pritzker" />
